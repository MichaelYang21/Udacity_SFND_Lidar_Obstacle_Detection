...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    for(pcl::PointCloud<pcl::PointXYZ>::Ptr cluster:cloudClusters)
    {
        ...
        renderPointCloud(viewer,cluster,"osbstCloud"+std::to_string(clusterId),colors[clusterId%colors.size()]);
        
        Box box=pointProcessor.BoundingBox(cluster);
        //renderBox(viewer,boxclusterId);
...
//void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    //renderPointCloud(viewer,inputCloud,"inputCloud");
    pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud=pointProcessorI->FilterCloud(inputCloud,0.20f,Eigen::Vector4f(-10,-6,-4,1),Eigen::Vector4f(30,6,2.5,1));
    renderPointCloud(viewer,filterCloud,"filterCloud");

    // pcl default segmentation solution
    ...
    */
    ...
    cout<<"starting clusterting"<<endl;

    //std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters =pointProcessorI->Clustering(segmentCloud.first,0.5,30,1000);

    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters =pointProcessorI->EucildeanClustering(segmentCloud.first,0.5,30,1000);
